...
class LMTasksLogist(val mc: MainContext) {
    private fun emptyCreep(creep: Creep, terminal: StructureTerminal): CreepTask? {
        if (creep.store.getUsedCapacity() != 0) {
            val resTransfer = creep.store.toMap().filter { it.value != 0 }.toList().firstOrNull()
            if (resTransfer != null) {
                return CreepTask(TypeOfTask.TransferTo, terminal.id, terminal.pos, resource = resTransfer.first, quantity = resTransfer.second)
            }
        }
        return null
    }
    ...
    private fun resourceForReactionsToLab(creep: Creep, terminal: StructureTerminal,lab: StructureLab,resLab: Pair<ResourceConstant, Int>):CreepTask? {
        var quantityTransfer: Int = min(lab.store.getFreeCapacity(resLab.first) ?: 0 ,resLab.second)
    ...
    fun newTask(creep: Creep):Boolean {
        ...
        if (creepTask == null) creepTask = this.emptyCreep(creep, terminal)
