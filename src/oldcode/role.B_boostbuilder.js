...
var role_B_boostbuilder = {
    ...
    run: function(creep) {
	    ...
	    if((creep.store.energy > 0 || objRoom.CalcRoleCreeps[22]!=0) && creep.memory.Task == 'repair' && creep.memory.isTask == 1) {
            var repair = Game.getObjectById(creep.memory.idRepair);
        ...
        if((creep.store.energy > 0 || objRoom.CalcRoleCreeps[22]!=0) && creep.memory.Task == 'build' && creep.memory.isTask == 1) {
            var build = Game.getObjectById(creep.memory.idRepair);
        ...
        if(creep.store.energy == 0 && creep.memory.isTask == 1 && objRoom.CalcRoleCreeps[22]==0) {
